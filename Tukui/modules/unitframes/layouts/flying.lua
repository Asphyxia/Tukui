...
if C["unitframes"].style ~= "Flying" then return end
...
local function Shared(self, unit)
	...
	if (unit == "player" or unit == "target") then
		...
		if T.lowversion then
			...
			panel:CreatePanel("Default", 260, 21, "BOTTOM", self, "BOTTOM", 0, 0)
		end
		...
		power:Width(209)
		if unit == "player" then
			power:Point("CENTER", health, "CENTER", 0, 0)
		...
		if (unit == "player") then
			...
			if T.level ~= MAX_PLAYER_LEVEL then
				...
				Experience:Size(5, 153)
				Experience:Point("BOTTOMRIGHT", TukuiChatBackgroundLeft, "BOTTOMRIGHT", 12, -19)
			...
			if T.level == MAX_PLAYER_LEVEL then
				...
				Reputation:Size(5, 153)
				Reputation:Point("BOTTOMRIGHT", TukuiChatBackgroundLeft, "BOTTOMRIGHT", 12, -19)
			...
			if C["unitframes"].classbar then
				if T.myclass == "DRUID" then
					...
					eclipseBar:Point("BOTTOM", self, "TOP", 0, 10)
				...
				if (T.myclass == "WARLOCK" or T.myclass == "PALADIN") then
                    ...
                    bars:Size(15, 10)
					bars:Point("RIGHT", health, "LEFT", -27, 0)
                ...
                Runes:Point("RIGHT", health, "LEFT", -37, 0)
                Runes:Size(30, 5)
				...
				local TotemBar = {}
					...
					for i = 1, 4 do
						...
						if (i == 1) then
					    TotemBar[i]:Point("RIGHT", health, "LEFT", -48, 0) else
					    TotemBar[i]:SetPoint("TOPLEFT", TotemBar[i-1], "TOPRIGHT", T.Scale(7), 0)
					end
					TotemBar[i]:SetStatusBarTexture(normTex)
					TotemBar[i]:SetHeight(T.Scale(17))
		...
		if (unit == "target" and C["unitframes"].targetauras) or (unit == "player" and C["unitframes"].playerauras) then
			...
			if (T.myclass == "SHAMAN" or T.myclass == "DEATHKNIGHT" or T.myclass == "PALADIN" or T.myclass == "WARLOCK") and (C["unitframes"].playerauras) and (unit == "player") then
				if T.lowversion then
					...
					buffs:SetPoint("TOPLEFT", self, "TOPLEFT", 0, 43)
				end
			else
				if T.lowversion then
					buffs:SetPoint("TOPLEFT", self, "TOPLEFT", 0, 26)
				else
					buffs:SetPoint("TOPLEFT", self, "TOPLEFT", -6, 37)
				end
			end
		...
		if (C["unitframes"].unitcastbar == true) then
			...
			if unit == "player" then
				...
				castbar:Point("BOTTOM", TukuiBar1, "TOP", 0, 22)
			elseif unit == "target" then
				if C["unitframes"].cbicons == true then
					castbar:SetWidth(205)
				else
					castbar:SetWidth(TukuiBar1:GetWidth() - 4)
				end
				castbar:SetHeight(17)
			...
			castbar.border:CreatePanel("Thin",1,1,"TOPLEFT", castbar, "TOPLEFT", -2, 2)
			...
			if C["unitframes"].cbicons == true then
				...
				castbar.button:SetTemplate("Thin")
	...
	if (unit == "focus") then
		...
		debuffs:Point('LEFT', self, 'RIGHT', 10, 14)
		debuffs.size = 20
		debuffs.num = 8
		debuffs.spacing = 2
	...
	if (unit == "focustarget") then
		...
		debuffs:SetHeight(26)
		debuffs:SetWidth(200)
		debuffs:Point('RIGHT', self, 'LEFT', -10, 14)
		debuffs.size = 26
		debuffs.num = 0
		debuffs.spacing = 2
	...
	if (unit and unit:find("arena%d") and C["arena"].unitframes == true) or (unit and unit:find("boss%d") and C["unitframes"].showboss == true) then
		...
		if (unit and unit:find("boss%d")) then
			...
			buffs:SetHeight(26)
			buffs:SetWidth(252)
			buffs:Point("TOPRIGHT", self, "TOPLEFT", -10, 3.5)
			buffs.size = 26
			buffs.num = 3
			buffs.spacing = 3
...
if T.lowversion then
	tot:SetPoint("RIGHT", target, "LEFT", -82, 20)
	tot:Size(186, 17)
else
	tot:SetPoint("RIGHT", target, "LEFT", -82, 20)
	tot:Size(105, 17)
...
local pet = oUF:Spawn('pet', "TukuiPet")
	pet:SetPoint("TOP", tot, "BOTTOM", 0, -30)
	pet:Size(105, 17)
