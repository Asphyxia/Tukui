...
local function Shared(self, unit)
	...
	if (unit == "player" or unit == "target") then
		...
		health.value = T.SetFontString(health, font, 8, "MONOCHROMEOUTLINE")
		...
		power.value = T.SetFontString(health,font, 8, "MONOCHROMEOUTLINE")
		...
		if unit == "player" then
			if C.unitframes.charportrait then
				classicon:CreatePanel("Default", 39, 39, "TOPLEFT", health, "TOPRIGHT", 5,2)
			else
				classicon:CreatePanel("Default", 39, 39, "TOPRIGHT", health, "TOPLEFT", -5,2)
			end
		elseif unit == "target" then
			if C.unitframes.charportrait then
				classicon:CreatePanel("Default", 39, 39, "TOPRIGHT", health, "TOPLEFT", -5,2)
		...
		if (unit == "player") then
			...
			FlashInfo.ManaLevel = T.SetFontString(FlashInfo, font, 8, "MONOCHROMEOUTLINE")
			...
			if C["unitframes"].classbar then
				if T.myclass == "DRUID" then
					...
					eclipseBarText:SetFont(font, 8, "MONOCHROMEOUTLINE")
			...
			-- deathknight runes
                ...
                Runes:Point("TOPLEFT", health, "TOPLEFT", 7, 8)
                Runes:Size(30, 5)
		...
		-- shaman totem bar
				...
				local TotemBar = {}
					...
					for i = 1, 4 do
						...
						if (i == 1) then
					    TotemBar[i]:Point("TOPLEFT", health, "TOPLEFT", 7, 8) else
		...
		if C["unitframes"].combatfeedback == true then
			...
			CombatFeedbackText = T.SetFontString(health, font, 8, "MONOCHROMEOUTLINE")
	...
	if (unit == "targettarget") then
		...
		local power = CreateFrame('StatusBar', nil, self)
		power:Height(3)
		power:Point("TOPLEFT", health, "BOTTOMLEFT", 9, 7)
	...
	if (unit and unit:find("arena%d") and C["arena"].unitframes == true) or (unit and unit:find("boss%d") and C["unitframes"].showboss == true) then
		...
		local power = CreateFrame('StatusBar', nil, self)
		power:Height(3)
		power:Point("TOPLEFT", health, "BOTTOMLEFT", 10, 8)
...
local pet = oUF:Spawn('pet', "TukuiPet")
	pet:SetPoint("LEFT", player, "RIGHT", 70, -30)
	pet:Size(129, 36)
...
local function Update(self, elapsed)
  TimeSinceLastUpdate = TimeSinceLastUpdate + elapsed	
	...
	if (TimeSinceLastUpdate > CustomBar_UpdateInterval) then
		CustomTukuiActionBarAnchor:Point( "TOPLEFT", TukuiPlayer, "BOTTOMLEFT", 40, -2)
	end
    TimeSinceLastUpdate = 0
end
