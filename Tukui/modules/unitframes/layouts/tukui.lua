...
local function Shared(self, unit)
	...
	if (unit == "player" or unit == "target") then
		...
		health.value = T.SetFontString(health, C.media.pixelfont, C["datatext"].fontsize, "MONOCHROMEOUTLINE")
		...
		if unit == "player" then
			if C.unitframes.charportrait then
				classicon:CreatePanel("Default", 28, 28, "TOPLEFT", health, "TOPRIGHT", 8, 3)
			else
				classicon:CreatePanel("Default", 28, 28, "TOPRIGHT", health, "TOPLEFT", -8, 3)
			end
		elseif unit == "target" then
			if C.unitframes.charportrait then
				classicon:CreatePanel("Default", 28, 28, "TOPRIGHT", health, "TOPLEFT", -8, 3)
		...
		if C["unitframes"].combatfeedback == true then
			...
			CombatFeedbackText = T.SetFontString(health, C.media.pixelfont, C["datatext"].fontsize, "MONOCHROMEOUTLINE")
...
if T.lowversion then
	tot:SetPoint("RIGHT", target, "LEFT", -69, 20)
	tot:Size(186, 18)
else
	tot:SetPoint("RIGHT", target, "LEFT", -69, 20)
	tot:Size(129, 36)
...
local pet = oUF:Spawn('pet', "TukuiPet")
	pet:SetPoint("LEFT", player, "RIGHT", 70, -10)
	pet:Size(129, 36)
