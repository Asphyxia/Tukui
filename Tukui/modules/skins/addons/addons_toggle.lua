...
for i = 1, 4 do
	...
	-- elseif i == 4 then 		-- Elvui ConfigUi
		...
		-- tab[i]:SetScript("OnEnter", function(self)
			...
			-- GameTooltip:ClearLines()
			-- if TLock[i] == true then
				-- GameTooltip:AddDoubleLine("ElvUi ConfigUi : ", HIDE,1,1,1,unpack(C["media"].statcolor))
			-- else
				-- GameTooltip:AddDoubleLine("ElvUi ConfigUi : ", SHOW,1,1,1,unpack(C["media"].statcolor))
			-- end
			-- GameTooltip:Show()  
			-- if TLock[i] == true then return end; 
			-- T.ToggleTab(self, TabIn[i], 4); 
