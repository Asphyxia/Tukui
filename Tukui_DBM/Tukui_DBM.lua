...
local buttonsize
if C.DBM_Skin.iconsize and type(C.DBM_Skin.iconsize)=="number" then
	buttonsize = C.DBM_Skin.iconsize
else
	buttonsize = 20	-- just to be safe
end
...
local function SkinBars(self)
	for bar in self:GetBarIterator() do
		if not bar.injected then
				...
				tbar:SetWidth(C.DBM_Skin.barwidth)
				frame:SetWidth(C.DBM_Skin.barwidth)
